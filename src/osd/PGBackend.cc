...
struct Trimmer : public ObjectModDesc::Visitor {
  ...
  void ec_omap(bool clear_omap, std::optional<ceph::buffer::list> omap_header,
    ...
    auto dpp = pg->get_parent()->get_dpp();
    ldpp_dout(dpp, 20) << __func__ << " generation is " << gen << " for oid " <<
  	soid.oid.name << " shard " << shard << " lost delete = " << lost_delete << dendl;
    ...
    if (!sinfo.is_nonprimary_shard(shard)) {
      ...
      if (lost_delete) {
        ...
        if (r != 0) {
          ...
          auto dpp = pg->get_parent()->get_dpp();
          ldpp_dout(dpp, 20) << __func__ << " skipping omap update for " << soid
                             << " gen=" << gen << " on shard " << shard
                             << " (object does not exist, lost_delete=true)" << dendl;
          should_update = false;
        }
      }
