...
TEST_P(OmapTest, OmapRecovery) {
  ...
  ret = rados.mon_command(cmd_out.str(), std::move(inbl), &outbl, nullptr);
  EXPECT_EQ(ret, 0);
  cmd_down << "{\"prefix\": \"osd down\", \"ids\": [\"" << victim_osd << "\"]}";
  ret = rados.mon_command(cmd_down.str(), std::move(inbl), &outbl, nullptr);
  EXPECT_EQ(ret, 0);
  ...
  for (int i = 0; i < 120; ++i) {
    ...
    rados.mon_command(std::move(cmd), std::move(inbl2), &outbl2, nullptr);
  ...
  rados.mon_command(cmd_scrub.str(), bufferlist(), &outbl, nullptr);
  ...
  for (int i = 0; i < 60; ++i) {
    bufferlist in, out;
    std::string cmd = "{\"prefix\": \"pg dump\", \"pgid\": \"" + pgid + "\"}";
    rados.mon_command(std::move(cmd), std::move(in), &out, nullptr);
  ...
  rados.mon_command(std::move(q_cmd), std::move(q_in), &q_out, nullptr);
  ...
  // 9. Bring Old Primary OSD Back In
  std::ostringstream cmd_in;
  bufferlist inbl3;
  cmd_in << "{\"prefix\": \"osd in\", \"ids\": [\"" << victim_osd << "\"]}";
  ret = rados.mon_command(cmd_in.str(), std::move(inbl3), &outbl, nullptr);
  EXPECT_EQ(ret, 0);

  turn_balancing_on();
}

TEST_P(OmapTest, ChangeUpmap) {
  SKIP_IF_CRIMSON();
  turn_balancing_off();
  bufferlist bl_write, omap_val_bl, xattr_val_bl;
  const std::string omap_key_1 = "key_a";
